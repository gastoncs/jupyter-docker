...
class DataCollectorXtb():
    ...
    def history_converter(self, history):
        ...
        df['YMD'] = df['ctm'].apply(lambda x: datetime.fromtimestamp(x / 1000).strftime('%Y%m%d'))
        df['YMDH'] = df['ctm'].apply(lambda x: datetime.fromtimestamp(x / 1000).strftime('%Y%m%d%H'))
    ...
    def add_spreads(self,df):
        ...
        spreads=pd.read_csv(f'{config.spread_folder}{self.symbol}_spreads.csv', parse_dates=['date'], index_col='date')  
        df['weekday']=df.index.day_name()
        df['hour']=df.index.hour
        df=df[df['weekday']!='Sunday']
        m_df=pd.merge(df.reset_index(), spreads, how='left',on=['weekday','hour']).set_index('Date').replace("", None).ffill()
