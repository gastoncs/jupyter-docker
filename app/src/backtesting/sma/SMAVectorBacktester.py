...
class SMAVectorBacktester(object):
    ...
    def __init__(self, symbol, SMA1, SMA2, start, end, dataCollector, refreshData):
    ...
    def get_data(self):
        ...
        '''
        if(self.refreshData):
            collector=self.dataCollector(symbol=self.symbol, start=self.start, end=self.end, period='1min',
                    spread_col=False, cols_to_save=['Open','Close','High','Low'])
    
            store = pd.HDFStore("data/smaData.h5", "w")  
            store.put("data", collector.data, format="table")  
        else:
            store = pd.HDFStore("data/smaData.h5", "r")  
            
        raw = store.select('/data', where="columns == Close")
        store.close() 
