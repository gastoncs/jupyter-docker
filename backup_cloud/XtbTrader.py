...
class XtbTrader():
    ...
    def set_target_order(self, position_type):

        '''
        Description
        ===============================================
        Method sending a target position
        :param

        *position_type-> str {'buy','sell'}

        '''
        positions_dict = {'sell': TransactionSide.SELL_STOP, 'buy': TransactionSide.BUY_STOP}

        order_type = positions_dict[position_type]

        self.current_order_num

        open_price = self.get_open_position_price(self.current_order_num)
        
        if position_type == 'buy':
            target_price = open_price - self.pips
        else:
            target_price = open_price + self.pips
        ...
        if self.trade_counter == 1:
            start_session = datetime.datetime.now().strftime("%a,%d %b, %Y, %H:%M:%S")
            print(
                f'SEND TARGET AT {start_session} |instrument: {self.instrument}| strategy: {self.strategy.__name__} | interval: {self.interval}')
            self.session_started = True
    ...
    def get_open_position_price(self, current_order):
                ...
                if trade_details['status'] == True:
                    if len(trade_details['returnData']) > 0:
                        if current_order == trade_details['returnData'][-1]['order2']:
                           returned=trade_details['returnData'][-1]['open_price']
